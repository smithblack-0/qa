...
This is shamelessly stolen from Yann Dubois.

See his work for the original
...
import numpy as np
import supertransformerlib.Attention
...
class HashFamily():
    ...
    def __init__(self, bins, mask_zero=False, moduler=None):
        ...
        self.bins = bins
        self.moduler = moduler if moduler else self._next_prime(np.random.randint(self.bins + 1, 2**31-1))
...
class HashEmbedding(nn.Module):
    ...
    def reset_parameters(self,
        ...
        if init_shared is None:
            def init_shared(x):
                nn.init.normal_(x, std=0.1)
                return x
        if init_importance is None:
            def init_importance(x):
                nn.init.normal_(x, std=0.0005)
                return x
